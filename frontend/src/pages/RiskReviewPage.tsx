import { Alert, Box, Card, Chip, CircularProgress, Tab, Table, TableBody, TableCell, TableHead, TableRow, Tabs, Typography } from '@mui/material'
...
export function RiskReviewPage() {
  const [reviewTab, setReviewTab] = useState<'OPEN' | 'CLEARED'>('OPEN')
  ...
  const { data = [], isLoading, isError } = useQuery({ queryKey: ['risk-reviews', company?.id, reviewTab], enabled: Boolean(company), queryFn: async () => (await api.get<RiskReviewSummary[]>('/risk-reviews', { params: { company_id: company!.id, status: reviewTab } })).data })
