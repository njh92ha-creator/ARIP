...
function LegacyVarianceSettings({ company }: { company?: Company }) {
  const [message, setMessage] = useState("");
...
type AviException = {
  ...
  rate_threshold: string | number;
  reason: string;
};

type CurrentVarianceProfile = {
  id: string;
  status: string;
  effective_from: string | null;
  effective_to: string | null;
  thresholds: Array<{ comparison: "MOM" | "YOY"; amount_threshold: string | number; rate_threshold: string | number; minimum_base_amount: string | number; trigger_mode: "ANY" | "ALL" }>;
  exceptions: AviException[];
};
...
function VarianceSettings({ company }: { company?: Company }) {
  ...
  const [minimum, setMinimum] = useState("100000000");
  const [mode, setMode] = useState<"ANY" | "ALL">("ANY");
  const [effectiveFrom, setEffectiveFrom] = useState("2023-01-01");
  const [effectiveTo, setEffectiveTo] = useState("2023-12-31");
  const [exceptions, setExceptions] = useState<AviException[]>([]);
  const [exceptionOpen, setExceptionOpen] = useState(false);
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const profile = useQuery({
    queryKey: ["variance-profile", company?.id],
    enabled: Boolean(company),
    queryFn: async () => (await api.get<CurrentVarianceProfile | null>("/variance-settings/current", { params: { company_id: company!.id } })).data,
  });

  useEffect(() => {
    if (!profile.data) return;
    setEffectiveFrom(profile.data.effective_from ?? "2023-01-01");
    setEffectiveTo(profile.data.effective_to ?? "2023-12-31");
    setExceptions(profile.data.exceptions ?? []);
    const threshold = profile.data.thresholds.find((item) => item.comparison === comparison) ?? profile.data.thresholds[0];
    if (!threshold) return;
    setAmount(String(threshold.amount_threshold));
    setRate(String(Number(threshold.rate_threshold) * 100));
    setMinimum(String(threshold.minimum_base_amount));
    setMode(threshold.trigger_mode);
  ...
  const simulationRows = [
    { name: "미지급금", amount: "+12.4억", rate: "42%", status: "AVI 점검", detail: "직전 월 대비 급격한 부채 증가가 감지되었습니다." },
    { name: "선급금", amount: "+8.1억", rate: "28%", status: "AVI 점검", detail: "신규 계약 관련 선급금 변동이 기준을 초과했습니다." },
    { name: "연구개발비", amount: "-5.5억", rate: "21%", status: "AVI 점검", detail: "프로젝트 종료에 따른 비용 정체가 확인되었습니다." },
  ];

  return <Stack spacing={3}>
    <Grid container spacing={3} alignItems="stretch">
      <Grid size={{ xs: 12, lg: 5 }}>
        <Card sx={{ ...cardSx, height: "100%" }}><CardContent sx={{ p: 3 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}><Typography variant="h6" fontWeight={700}>계정 증감 기준 (AVI)</Typography><HelpOutline sx={{ color: colors.secondary }} /></Stack>
          <Box component="form" onSubmit={async (event) => { event.preventDefault(); setMessage(""); setError(""); try {
            const common = { amount_threshold: amount, rate_threshold: Number(rate) / 100, minimum_base_amount: minimum, trigger_mode: mode };
            await api.put(`/variance-settings/current/${company.id}`, { company_id: company.id, name: "기본 AVI", approve: true, effective_from: effectiveFrom, effective_to: effectiveTo, exceptions, thresholds: [{ comparison: "MOM", ...common }, { comparison: "YOY", ...common }] });
            await profile.refetch(); setMessage("AVI 기준이 승인 상태로 저장되었습니다.");
          } catch { setError("AVI 기준 저장에 실패했습니다. 입력값과 연결 상태를 확인해 주세요."); } }}>
            <Grid container spacing={2.5}>
              <Grid size={{ xs: 12, sm: 6 }}><Field label="기준 회사"><TextField value={`${company.company_name} (${company.company_code})`} fullWidth slotProps={{ input: { readOnly: true } }} sx={smallFieldSx} /></Field></Grid>
              <Grid size={{ xs: 12, sm: 6 }}><Field label="적용 기간"><Stack direction="row" spacing={1}><TextField type="date" value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} fullWidth sx={smallFieldSx} /><TextField type="date" value={effectiveTo} onChange={(event) => setEffectiveTo(event.target.value)} fullWidth sx={smallFieldSx} /></Stack></Field></Grid>
              ...
              <Grid size={{ xs: 12, sm: 6 }}><Field label="증감 금액 기준"><TextField type="number" value={amount} onChange={(event) => setAmount(event.target.value)} fullWidth sx={smallFieldSx} /></Field></Grid>
              <Grid size={{ xs: 12, sm: 6 }}><Field label="증감률 기준"><TextField type="number" value={rate} onChange={(event) => setRate(event.target.value)} fullWidth sx={smallFieldSx} /></Field></Grid>
              <Grid size={12}><Field label="최소 비교 금액"><TextField type="number" value={minimum} onChange={(event) => setMinimum(event.target.value)} fullWidth sx={smallFieldSx} /></Field></Grid>
              <Grid size={12}><Field label="Trigger Mode"><Segmented value={mode} onChange={(value) => setMode(value as "ANY" | "ALL")} options={[["ANY", "하나 이상 충족 (OR)"], ["ALL", "모두 충족 (AND)"]]} /></Field></Grid>
            </Grid>
            {saved && <Alert severity="info" sx={{ mt: 2 }}>현재 저장 상태: {saved.status === "APPROVED" ? "승인" : saved.status} · 적용 기간: {saved.effective_from} ~ {saved.effective_to}</Alert>}
            {error && <Alert severity="error" sx={{ mt: 2 }}>{error}</Alert>}{message && <Alert severity="success" sx={{ mt: 2 }}>{message}</Alert>}
            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 2.5 }}><Button type="submit" variant="contained" sx={{ bgcolor: colors.primary }}>설정 적용</Button></Stack>
          </Box>
        </CardContent></Card>
      </Grid>
      <Grid size={{ xs: 12, lg: 7 }}><Stack spacing={3} sx={{ height: "100%" }}>
        <AviExceptionTable exceptions={exceptions} onAdd={() => setExceptionOpen(true)} />
        <AviSimulationPreview rows={simulationRows} onDetails={() => setDetailsOpen(true)} />
      </Stack></Grid>
    </Grid>
    <Dialog open={exceptionOpen} onClose={() => setExceptionOpen(false)} fullWidth maxWidth="sm"><Box component="form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); setExceptions((items) => [...items, { account_name: String(data.get("account_name")), account_group: String(data.get("account_group")), comparison: data.get("comparison") as "MOM" | "YOY", amount_threshold: String(data.get("amount_threshold")), rate_threshold: Number(data.get("rate_threshold")) / 100, reason: String(data.get("reason")) }]); setExceptionOpen(false); }}><DialogTitle>계정별 예외 기준 추가</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}><TextField name="account_name" label="계정명" required fullWidth /><TextField name="account_group" label="계정 분류" fullWidth /><TextField name="comparison" label="비교 기준" select defaultValue="MOM" fullWidth><MenuItem value="MOM">MoM</MenuItem><MenuItem value="YOY">YoY</MenuItem></TextField><TextField name="amount_threshold" label="증감 금액 기준" type="number" required fullWidth /><TextField name="rate_threshold" label="증감률 기준(%)" type="number" required fullWidth /><TextField name="reason" label="사유/조치" multiline minRows={3} required fullWidth /></Stack></DialogContent><DialogActions><Button onClick={() => setExceptionOpen(false)}>취소</Button><Button type="submit" variant="contained">등록</Button></DialogActions></Box></Dialog>
    <Dialog open={detailsOpen} onClose={() => setDetailsOpen(false)} fullWidth maxWidth="md"><DialogTitle>AVI 적용 결과 상세</DialogTitle><DialogContent><Stack spacing={2}>{simulationRows.map((item) => <Card key={item.name} variant="outlined"><CardContent><Stack direction="row" justifyContent="space-between"><Typography fontWeight={700}>{item.name}</Typography><Chip label={item.status} size="small" /></Stack><Typography sx={{ mt: 1 }}>{item.amount} · {item.rate}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>{item.detail}</Typography><Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 1 }}>현재 적용 기준: {money(amount)} / {rate}%</Typography></CardContent></Card>)}</Stack></DialogContent><DialogActions><Button onClick={() => setDetailsOpen(false)}>닫기</Button></DialogActions></Dialog>
  </Stack>;
}

function AviExceptionTable({ exceptions, onAdd }: { exceptions: AviException[]; onAdd: () => void }) {
  return <Card sx={{ ...cardSx, flex: 1 }}><Box sx={cardHeaderSx}><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography variant="h6" fontWeight={700}>계정별 예외 기준</Typography><Typography variant="caption" color="text.secondary">개별 계정에 글로벌 기준 대신 별도 증감 임계치를 적용합니다.</Typography></Box><Button variant="outlined" startIcon={<Add />} onClick={onAdd} sx={{ borderColor: colors.primary, color: colors.primary }}>예외 기준 추가</Button></Stack></Box><TableContainer><Table size="small"><TableHead><TableRow>{["계정/분류", "비교 기준", "증감 금액", "증감률", "사유/조치"].map((label) => <TableCell key={label}>{label}</TableCell>)}</TableRow></TableHead><TableBody>{exceptions.length ? exceptions.map((item, index) => <TableRow key={`${item.account_name}-${index}`}><TableCell><Typography fontWeight={700}>{item.account_name}</Typography><Typography variant="caption" color="text.secondary">{item.account_group}</Typography></TableCell><TableCell>{item.comparison}</TableCell><TableCell>{Number(item.amount_threshold).toLocaleString("ko-KR")}</TableCell><TableCell>{Number(item.rate_threshold) * 100}%</TableCell><TableCell>{item.reason}</TableCell></TableRow>) : <TableRow><TableCell colSpan={5} align="center" sx={{ py: 4, color: colors.secondary }}>등록된 예외 기준이 없습니다.</TableCell></TableRow>}</TableBody></Table></TableContainer></Card>;
}

function AviSimulationPreview({ rows, onDetails }: { rows: Array<{ name: string; amount: string; rate: string; status: string; detail: string }>; onDetails: () => void }) {
  return <Card sx={cardSx}><CardContent sx={{ p: 2.5 }}><Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2.5 }}><Stack direction="row" spacing={1} alignItems="center"><Typography variant="h6" fontWeight={700}>적용 결과 미리보기</Typography><Chip label="CURRENT RULE SIMULATION" size="small" sx={{ bgcolor: colors.primarySoft, color: colors.primary, fontSize: 10 }} /></Stack><Button size="small" onClick={onDetails}>상세 결과 보기</Button></Stack><Grid container spacing={2}>{rows.map((item) => <Grid key={item.name} size={{ xs: 12, md: 4 }}><Box sx={{ p: 2, bgcolor: colors.canvas, border: `1px solid ${colors.border}`, borderRadius: 1 }}><Typography variant="caption" fontWeight={700} color="text.secondary">{item.name}</Typography><Typography sx={{ fontSize: 22, fontWeight: 700, mt: 1 }}>{item.amount} <Typography component="span" variant="caption" color="error.main">{item.rate}</Typography></Typography><Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 0.5 }}>{item.detail}</Typography></Box></Grid>)}</Grid></CardContent></Card>;
}
