    ...
    <Box component="section" sx={{ width: { xs: '100%', lg: '48%' }, minWidth: 0, flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', overflowY: 'auto', p: { xs: 3, sm: 6 }, bgcolor: '#F8FAFC' }}>
      <Box sx={{ width: '100%', maxWidth: 440, p: { xs: 4, sm: 5 }, bgcolor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '8px', boxShadow: '0 1px 3px rgba(16, 24, 40, 0.08)' }}>
        ...
        <Box component="form" onSubmit={submit} autoComplete="off"><Stack spacing={2.5}>
          ...
          <Box><Typography component="label" htmlFor="login-password" sx={{ display: 'block', color: '#101828', fontSize: 14, fontWeight: 500, mb: 0.75 }}>비밀번호</Typography><TextField id="login-password" name="arip-login-password" fullWidth required type={showPassword ? 'text' : 'password'} value={password} onFocus={() => setPasswordEditable(true)} onChange={(event) => setPassword(event.target.value)} autoComplete="off" slotProps={{ input: { readOnly: !passwordEditable, endAdornment: <IconButton aria-label="비밀번호 보기" edge="end" size="small" onClick={() => setShowPassword((visible) => !visible)} sx={{ color: '#667085', mr: 0.25 }}>{showPassword ? <VisibilityOffOutlined fontSize="small" /> : <VisibilityOutlined fontSize="small" />}</IconButton> } }} sx={fieldSx} /></Box>
          {mode === 'signup' && <Box><Typography component="label" htmlFor="confirm-password" sx={{ display: 'block', color: '#101828', fontSize: 14, fontWeight: 500, mb: 0.75 }}>비밀번호 확인</Typography><TextField id="confirm-password" name="arip-confirm-password" fullWidth required type={showPassword ? 'text' : 'password'} value={confirmPassword} onFocus={() => setPasswordEditable(true)} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="off" slotProps={{ input: { readOnly: !passwordEditable } }} sx={fieldSx} /></Box>}
