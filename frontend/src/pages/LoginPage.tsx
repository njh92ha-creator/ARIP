...
export function LoginPage() {
  ...
  return <Box component="main" sx={{ width: '100%', height: '100vh', minHeight: 720, display: 'flex', overflow: 'hidden', bgcolor: '#F8FAFC', fontFamily: '"Noto Sans KR", sans-serif' }}>
    <Box component="section" sx={{ position: 'relative', width: '52%', flexShrink: 0, display: { xs: 'none', lg: 'flex' }, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', p: 6, background: 'linear-gradient(135deg,#E0F2FE 0%,#BAE6FD 100%)', '@keyframes aripDotDrift': { '0%': { backgroundPosition: '0 0' }, '100%': { backgroundPosition: '48px 48px' } }, '@keyframes aripBrandFloat': { '0%, 100%': { transform: 'translateY(0)' }, '50%': { transform: 'translateY(-10px)' } }, '@keyframes aripBrandPulse': { '0%': { transform: 'scale(.72)', opacity: 0 }, '28%': { opacity: .48 }, '100%': { transform: 'scale(1.8)', opacity: 0 } }, '@keyframes aripFeaturePulse': { '0%, 100%': { transform: 'translateY(0)', boxShadow: '0 1px 2px rgba(14,42,86,.08)' }, '50%': { transform: 'translateY(-6px)', boxShadow: '0 10px 22px rgba(31,111,213,.22)' } }, '@keyframes aripAurora': { '0%, 100%': { transform: 'translate3d(-8%,-5%,0) scale(1)' }, '50%': { transform: 'translate3d(10%,8%,0) scale(1.15)' } }, '&::before': { content: '""', position: 'absolute', inset: 0, opacity: .13, backgroundImage: 'radial-gradient(#1F6FD5 .7px,transparent .7px)', backgroundSize: '24px 24px', animation: 'aripDotDrift 12s linear infinite' }, '&::after': { content: '""', position: 'absolute', width: 520, height: 520, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,255,255,.62) 0%, rgba(255,255,255,.18) 42%, transparent 70%)', animation: 'aripAurora 9s ease-in-out infinite', pointerEvents: 'none' }, '@media (prefers-reduced-motion: reduce)': { '& *, &::before, &::after': { animation: 'none !important' } } }}>
      <Stack alignItems="center" sx={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 512, textAlign: 'center' }}>
        <Box sx={{ position: 'relative', width: 96, height: 96, mb: 4, animation: 'aripBrandFloat 3.8s ease-in-out infinite' }}><Box sx={{ position: 'absolute', width: 128, height: 128, left: -16, top: -16, border: '2px solid rgba(31,111,213,.5)', borderRadius: '50%', animation: 'aripBrandPulse 3.8s ease-out infinite' }} /><Box sx={{ position: 'absolute', inset: 0, borderRadius: 3, bgcolor: '#1F6FD5', opacity: .1, transform: 'rotate(45deg)' }} /><Box sx={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: '#1F6FD5' }}><svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2 2 7l10 5 10-5-10-5Z" /><path d="m2 17 10 5 10-5M2 12l10 5 10-5" /></svg></Box></Box>
