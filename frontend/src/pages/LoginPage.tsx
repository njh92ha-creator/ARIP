...
export function LoginPage() {
  ...
  return <Box component="main" sx={{ width: '100%', height: '100vh', minHeight: 720, display: 'flex', overflow: 'hidden', bgcolor: '#F8FAFC', fontFamily: '"Noto Sans KR", sans-serif' }}>
    <Box component="section" sx={{ position: 'relative', width: '52%', flexShrink: 0, display: { xs: 'none', lg: 'flex' }, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', p: 6, background: 'linear-gradient(135deg,#E0F2FE 0%,#BAE6FD 100%)', '&::before': { content: '""', position: 'absolute', inset: 0, opacity: .1, backgroundImage: 'radial-gradient(#1F6FD5 .5px,transparent .5px)', backgroundSize: '24px 24px' } }}>
      <Stack alignItems="center" sx={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 512, textAlign: 'center' }}>
        <Box sx={{ position: 'relative', width: 96, height: 96, mb: 4 }}><Box sx={{ position: 'absolute', inset: 0, borderRadius: 3, bgcolor: '#1F6FD5', opacity: .1, transform: 'rotate(45deg)' }} /><Box sx={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: '#1F6FD5' }}><svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2 2 7l10 5 10-5-10-5Z" /><path d="m2 17 10 5 10-5M2 12l10 5 10-5" /></svg></Box></Box>
        ...
        <Box sx={{ width: '100%', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', columnGap: 4, mt: 6, pt: 6, borderTop: '1px solid #BFDBFE' }}>{brandFeatures.map(({ label, icon }) => <Stack key={label} alignItems="center"><Box sx={{ width: 48, height: 48, borderRadius: '50%', bgcolor: '#FFF', color: '#1F6FD5', display: 'grid', placeItems: 'center', boxShadow: '0 1px 2px rgba(14,42,86,.08)', mb: 1.5 }}>{icon}</Box><Typography sx={{ color: '#0E2A56', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>{label}</Typography></Stack>)}</Box>
      </Stack>
    </Box>
